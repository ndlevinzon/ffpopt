...
4. Install ffpopt source and python dependencies via pip. The dependencies are separated into "groups".
   ...
   .. code-block:: bash
       ...
       ACADEMIC=TRUE python3 -m pip install --group tensorflow --extra-index-url https://download.pytorch.org/whl/cu121 --extra-index-url https://pypi.nvidia.com --no-cache-dir .
       mamba deactivate

       mamba activate ffpopt-pytorch
       CFLAGS="-Wno-array-bounds" CXXFLAGS="-Wno-array-bounds -include cstdint" ACADEMIC=TRUE python3 -m pip install --group pytorch --extra-index-url https://download.pytorch.org/whl/cu121 --extra-index-url https://pypi.nvidia.com --find-links https://data.dgl.ai/wheels/cu121/repo.html --no-cache-dir .
       mamba deactivate
...
7. You will likely need to set some environmental variables.

   .. code-block:: bash
		   
      export AMBERHOME="${PWD}/miniforge3/"
      export QUICK_BASIS="${PWD}/miniforge3/AmberTools/src/quick/basis"

8. There are example module files in ffpopt/modulefiles.
