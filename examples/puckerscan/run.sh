...
# NDim wavefront (--nproc below) uses slim-job spawn workers and the
# persistent ASE calculator. Add --geometric-opt for in-process geomeTRIC
# (FFPOPT_GEOMETRIC_SUBPROCESS=1 restores the CLI engine). The bin script
# is already wrapped in if __name__ == "__main__".
...
if [ -e ../../src/python/bin/ffpopt-NDimWavefront.py ]; then
	...
	python3 ../../src/python/bin/ffpopt-NDimWavefront.py \
		...
		--wf-max-levels=100 \
		--nproc=12 2> /dev/null
	# --nproc uses slim-job spawn workers; the bin script is already
