...
class Constraint(object):
    ...
    ----------
    name : str
        constraint type: "bond", "angle", "dihed", "puckerx", "puckery", "freeze"
    idxs : list of int
        A list of 2, 3, 4, or 5 integers representing the indices of atoms
        involved in the constraint. For a bond, it should be 2 integers, for an angle 3 integers, and for a dihedral 4 integers. If name=="freeze", the list can be as long as you want
    ...
    def to_geometric(self):
        ...
        """
        if self.name == "freeze":
            s = "\n".join(["xyz %i"%(x+1) for x in self.idxs])
        else:
    ...
    def isper(self):
        return len(self.idxs) == 4 and self.name != "freeze"
...
def to_ase(cons):
    ...
    for con in cons:
        # 1. Process Cartesian freezes first
        if con.name == "freeze":
            frozen_atoms.extend(con.idxs)
            continue  
            
        # 2. Sort internal coordinates by geometric dimension
        if len(con.idxs) == 2:
            bonds.append([con.value, con.idxs])
        elif len(con.idxs) == 3:
            angles.append([con.value, con.idxs])
    ...
    for key in pairs:
        ...
        # Fixed: Explicit positional indices restored
        diheds.append([v1, [idxs[4], idxs[0], idxs[1], idxs[2]]])
        diheds.append([v3, [idxs[1], idxs[2], idxs[3], idxs[4]]])
        
    # 4. Generate the structured list of constraint instances
    ase_constraints = []
    
    if bonds or angles or diheds:
        ase_constraints.append(
            FixInternals(bonds=bonds, angles_deg=angles, dihedrals_deg=diheds)
        )
        
    if frozen_atoms:
        unique_frozen = list(set(frozen_atoms))
        ase_constraints.append(FixAtoms(indices=unique_frozen))
        
    # 5. Always return a consistent list format for the user
    return ase_constraints
