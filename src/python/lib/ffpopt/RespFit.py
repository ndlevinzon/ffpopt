...
def espaloma_charge(molecule, total_charge=None):
    ...
    # 3. Handle GPU routing dynamically
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    
    # Safely move graph and model to the active device
    graph = graph.to(device)
    if device.startswith("cuda"):
        model = model.to(device) # .to(device) works for both cpu and cuda

    # Use the context manager to force internal library allocations onto the active device
    with torch.device(device):
        for layer in model:
            if layer.__class__.__name__ == "ChargeEquilibrium":
                graph = layer(graph, total_charge=float(total_charge))
            else:
                graph = layer(graph)
...
def __RunRespFit_backend\
        (*,
         inp: str,
         out: str,
         confs: list, # list of cpefit.Conformer
         respf: bool = False,
         espaloma: bool = False,
         hilfiker: bool = False,
         nofit: bool = False,
...
def RunRespFit(*,
    ...
    if confdep or len(args.out) > 0:
        qdepqs = []
        for iconf,conf in enumerate(confs):
            myargs = copy.deepcopy(args)
            myargs.confs = [conf]
            qs = __RunRespFit_backend(**vars(myargs))
            los[iconf].data["charges"] = [q for q in qs]
