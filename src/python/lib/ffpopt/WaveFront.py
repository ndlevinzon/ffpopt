...
def _init_1d_worker(los, con, template_struct, node_cls) -> None:
    """Pool initializer: share los / constraint templates once per worker."""
    from . WavefrontIpc import clear_los_calc

    clear_los_calc(los)
    _WORKER["los"] = los
    _WORKER["con"] = copy.deepcopy(con)
    _WORKER["template"] = copy.deepcopy(template_struct)
    _WORKER["node_cls"] = node_cls


def _struct_from_coords(coords):
    from . WavefrontIpc import clone_struct_geometry

    return clone_struct_geometry(_WORKER["template"], coords, ene=0.0, frcs=None)


def _run_node_job(job: dict) -> dict:
    """Worker entry: slim coords job in -> slim result out (no ``los``)."""
    from . WavefrontIpc import slim_node_result

    cls = _WORKER.get("node_cls") or WavefrontNode
    con = _WORKER.get("con")
    if job.get("con") is not None:
        try:
            from . Constraints import Constraint
            con = Constraint.from_dict(job["con"])
        except Exception:
            con = _WORKER.get("con")
    node = cls.from_job(job, _WORKER["los"], con)
    node.calculate()
    return slim_node_result(node)


def _run_node(node: "WavefrontNode") -> "WavefrontNode":
    """ Optimize a single node and return it (serial / legacy full pickle).
...
class WavefrontNode:
    ...
    @classmethod
    def from_job(cls, job: dict, los: ListOfStruct, con=None) -> "WavefrontNode":
        ...
        if job.get("coords") is not None:
            ...
            struct = copy.deepcopy(_WORKER.get("template"))
        node = cls(
            los=los,
            struct=struct,
            con=con,
    ...
    def to_job(self) -> dict:
        ...
        data = getattr(self.struct, "data", None) or {}
        coords = data.get("positions")
        if coords is not None:
            coords = np.asarray(coords, dtype=float)
        con0 = self.constraints[0] if self.constraints else None
        con_dict = None
        if con0 is not None and hasattr(con0, "to_dict"):
            con_dict = con0.to_dict()
        return {
            "angle": self.angle,
            "coords": coords,
            "level": self.level,
    ...
    def apply_result(self, result: dict) -> None:
        from . WavefrontIpc import apply_slim_node_result

        apply_slim_node_result(self, result)
    ...
    def replace_with_pickle(self) -> None:
        ...
        """
        from . WavefrontIpc import replace_node_with_pickle
    ...
    def _write_checkpoint(self) -> None:
        ...
        """
        from . WavefrontIpc import write_node_pickle
    ...
    def _precheck_geometry(self, min_dist: float = 0.8) -> bool:
        ...
        try:
            ...
            from . WavefrontIpc import has_nonbonded_clash
            ...
            bond_list = self.struct.data.get("bonds") or []
            clashed, i, j, dist = has_nonbonded_clash(
                myatoms.get_positions(), bond_list, min_dist=min_dist
            )
            if clashed:
                print(
...
class Wavefront:
    ...
    def calculate(self) -> None:
        ...
        between runs at ``nproc > 1``. geomeTRIC runs in-process against a
        persistent ASE calculator by default (set ``FFPOPT_GEOMETRIC_SUBPROCESS=1``
        for the legacy CLI). ``nproc`` workers each still occupy one process;
        pin math threads with ``OMP_NUM_THREADS`` if unset.
        ...
        from . AseEngine import calc_cache_key
        from . NondaemonPool import acquire_wavefront_pool
        from . WavefrontIpc import (
        ...
        if self.nproc > 1:
            ...
            for level in self.levels:
                ...
                if template is not None:
                    break
            if template is None:
                structs = getattr(self.los, "structs", None)
                template = structs[0] if structs else self.los
            reuse_key = (
                int(self.nproc),
                "1d",
                node_cls.__module__,
                node_cls.__name__,
            ) + tuple(calc_cache_key(self.los, template))
            pool, owns_pool = acquire_wavefront_pool(
                self.nproc,
                initializer=_init_1d_worker,
                initargs=(self.los, self.con, template, node_cls),
                reuse_key=reuse_key,
            )
                ...
                if in_flight:
                    ...
                    for async_result in list(in_flight):
                        if async_result.ready():
                            parent = in_flight.pop(async_result)
                            result = async_result.get()
                            parent.apply_result(result)
                            pending.extend(self._on_complete(parent))
                            since_checkpoint += 1
                            progressed = True
                    if not progressed:
                        time.sleep(0.1)
...
def run_dihed_wavefront(
    ...
    **standard_kwargs,
    ...
    require_main_guard_for_spawn("run_dihed_wavefront")
