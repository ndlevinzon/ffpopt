...
def _init_nd_worker(los, conlist, reslist, template_struct, node_cls) -> None:
...
def _run_node_job(job: dict) -> dict:
    ...
    node = cls.from_job(
        job, _WORKER["los"], _WORKER.get("conlist"), _WORKER.get("reslist")
    )
    node.calculate()
...
class WavefrontNode(object):
    ...
    def _precheck_geometry(self, min_dist: float = 0.8) -> bool:
        ...
        try:
            ...
            myatoms = ApplyConstraints(myatoms, self.conlist.cons, graph=self.struct.GetGraph())
...
class Wavefront(object):
    ...
    def calculate_threads(self) -> None:
        ...
        if self.nproc > 1:
            ...
            con_key = tuple(
                tuple(getattr(c, "idxs", ()) or ())
                for c in (self.conlist or [])
            )
            reuse_key = (
                int(self.nproc),
                "nd",
            ...
            pool, owns_pool = acquire_wavefront_pool(
                self.nproc,
                initializer=_init_nd_worker,
                initargs=(self.los, self.conlist, self.reslist, template, node_cls),
                reuse_key=reuse_key,
            )
