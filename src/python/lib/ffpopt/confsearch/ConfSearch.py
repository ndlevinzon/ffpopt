...
def ConformerSearch(fnameormol,outbasename,nconf,nkeep,mmff94,maxiter,rmstol,quiet):
    ...
    from .. Struct import parse_filename_charge
    inplos = None

    # fname,charge = parse_filename_charge(fnameormol)
    # if Path(fname).suffix in [ ".json", ".pdb", ".mol2", ".xyz" ]:
    ...
    if isinstance(fnameormol, ListOfStruct):
        inplos = deepcopy( fnameormol )
    else:
        inplos = ListOfStruct.from_file( fnameormol )
    m = inplos.structs[0].GetRDKitAtoms()
